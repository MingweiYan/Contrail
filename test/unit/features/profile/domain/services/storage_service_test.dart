...
void main() {
  group('StorageServiceInterface', () {
    ...
    setUp(() {
      ...
      when(() => mockStorageService.readData(any())).thenAnswer((_) async => {'test': 'data'});
      when(() => mockStorageService.writeData(any(), any())).thenAnswer((_) async => true);
      when(() => mockStorageService.getReadPath()).thenAnswer((_) async => '/test/path');
      when(() => mockStorageService.setWritePath(any())).thenAnswer((_) async => '/test/path');
      when(() => mockStorageService.listFiles()).thenAnswer((_) async => [testFile]);
      when(() => mockStorageService.deleteFile(any())).thenAnswer((_) async => true);
      when(() => mockStorageService.checkPermissions()).thenAnswer((_) async => true);
      when(() => mockStorageService.openFileSelector()).thenAnswer((_) async => '/test/file.txt');
      when(() => mockStorageService.openDirectorySelector()).thenAnswer((_) async => '/test/dir');
      when(() => mockStorageService.getFileSize(any())).thenAnswer((_) async => 1024);
      when(() => mockStorageService.getFileLastModified(any())).thenAnswer((_) async => DateTime.now());
    });
    ...
    test('writeData should return boolean', () async {
      final result = await mockStorageService.writeData('test.json', {'test': 'data'});
      expect(result, isA<bool>());
      verify(() => mockStorageService.writeData('test.json', {'test': 'data'})).called(1);
    });
