...
void main() {
  group('HabitDetailStatisticsProvider calculateHabitStats', () {
    ...
    test('Weekly: completion uses current week vs targetDays (no multiplication)', () {
      final habit = Habit(id: 'h', name: 'w', trackTime: false, targetDays: 3, cycleType: CycleType.weekly);
      final provider = HabitDetailStatisticsProvider(habit);
      final now = DateTime.now();
      final monday = now.subtract(Duration(days: (now.weekday - 1))); // Monday
      habit.dailyCompletionStatus[DateTime(monday.year, monday.month, monday.day)] = true;
      habit.dailyCompletionStatus[DateTime(monday.year, monday.month, monday.day + 1)] = true;
      final stats = provider.calculateHabitStats();
      expect(stats['targetDays'], 3);
      expect(stats['completedDays'], 2);
      expect((stats['completionRate'] as double) > 0.6 && (stats['completionRate'] as double) < 0.67, true);
    });

    test('Monthly: completion uses per-month target (not doubled in 31-day months)', () {
      final habit = Habit(id: 'h2', name: 'm', trackTime: false, targetDays: 12, cycleType: CycleType.monthly);
      final provider = HabitDetailStatisticsProvider(habit);
      final now = DateTime.now();
      for (int d = 1; d <= 15; d++) {
      ...
      expect((stats['completionRate'] as double) > 1.2 && (stats['completionRate'] as double) < 1.26, true);
    });
